...
for data_name in data_list:
    ...
    np.savez(
        os.path.join(folder_name, data_name + "_MCMC_blackjax.npz"),
...
# data_list = ["spam", "krkp", "ionosphere", "mushroom"]
# for data_name in data_list:
#     mici_data = np.load(data_name + "_MCMC_mici.npz")
#     blackjax_data = np.load(data_name + "_MCMC_blackjax.npz")
...
#     plt.scatter(variances_mici, variances_blackjax)
#     # add the diagonal
#     var_min = min(variances_mici.min(), variances_blackjax.min())
#     var_max = max(variances_mici.max(), variances_blackjax.max())
#     plt.plot([var_min, var_max], [var_min, var_max], "r-")
#     plt.title(f"{data_name}: Mici vs Blackjax")
