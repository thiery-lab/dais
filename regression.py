"""
This script produces the figure for the logistic regression examples in
van den Boom, Cremaschi and Thiery (2024, arXiv:2404.18556v1).
"""
import pickle
import time
...
# Data sets with associated parameters
# The Hamiltonian Monte Carlo parameter `step_size` was chosen such that the
# acceptance probability is neither too high nor too low.
data_sets = dict(
    spam=dict(url="spambase/spambase", S=10**4, step_size=0.07),
    krkp=dict(
        url="chess/king-rook-vs-king-pawn/kr-vs-kp", S=2 * 10**3,
        step_size=0.08
    ),
    ionosphere=dict(url="ionosphere/ionosphere", S=5 * 10**5, step_size=0.3),
    mushroom=dict(url="mushroom/agaricus-lepiota", S=2 * 10**5, step_size=0.2),
)
...
for data_name in data_sets.keys():
    ...
    integrator = mici.integrators.LeapfrogIntegrator(
        system, step_size=data_sets[data_name]["step_size"]
    )
    
    sampler = mici.samplers.DynamicMultinomialHMC(system, integrator, rng)

    init_state, chains, chain_stats = sampler.sample_chain(
        n_sample = MCMC_iter, init_state = init_state
    )
...
# Save `data_sets` for use with `regression_no_Stein.py`.
pickle.dump(data_sets, open("regression.p", "wb"))
